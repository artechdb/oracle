...
SPOOL healthcheck_report.html

-- Oracle 19c RAC Health Check HTML Report
prompt <h1>Oracle 19c RAC Database Health Check Report</h1>


prompt <hr><h2>prompt Note1: Session Information for Health Check</h2>
...
prompt <hr><h2>prompt Note2: Database Recycle Bin Status</h2>
...
prompt <hr><h2>prompt Note: Do not modify any inspection results</h2>
...
prompt <hr><h2>prompt Note that statistics gathering is global for the database AND  is persistent across instance starts</h2>
...
prompt <hr><h2>prompt NOTE: If less than95%,,or Adjust Database Parametersshared_pool_size</h2>
...
prompt <hr><h2>Cluster Nodes Status (GV$INSTANCE)</h2>
...
prompt <hr><h2>ASM Disk Group Usage (if using ASM)</h2>
COLUMN name FORMAT A20
COLUMN state FORMAT A10
COLUMN type FORMAT A10
SELECT name, state, type, total_mb, free_mb FROM v$asm_diskgroup;

prompt <hr><h2>RAC Interconnect Usage (GV$GES_BLOCKING_ENQUEUE)</h2>
SELECT inst_id, resource_name, requesting_session_id, blocking_session_id
FROM gv$ges_blocking_enqueue;

prompt <hr><h2>PDB Status (CDB$PDBS)</h2>
COLUMN pdb_name FORMAT A20
SELECT pdb_name, status FROM cdb_pdbs;

prompt <hr><h2>PGA/SGA Summary per Instance</h2>
...
prompt <hr><h2>Invalid Database Objects</h2>
...
prompt <hr><h2>Flashback Database Configuration</h2>
SELECT flashback_on, log_mode FROM v$database;

prompt <hr><h2>Archive Log Mode & Destinations</h2>
SELECT destination, status, target, error FROM v$archive_dest;

prompt <hr><h2>Undo Tablespace Usage</h2>
COLUMN tablespace_name FORMAT A20
SELECT tablespace_name, status, contents
FROM dba_tablespaces
WHERE contents = 'UNDO';

prompt <hr><h2>Blocking Sessions</h2>
COLUMN blocker FORMAT A15
COLUMN waiter FORMAT A15
SELECT s1.username || ' (' || s1.sid || ')' AS blocker,
       s2.username || ' (' || s2.sid || ')' AS waiter,
       s2.event, s2.seconds_in_wait
FROM gv$session s1, gv$session s2
WHERE s1.blocking_session = s2.sid
  AND s1.inst_id = s2.inst_id
  AND s1.sid != s2.sid;

prompt <hr><h2>Data Guard Configuration</h2>
SELECT database_role, protection_mode, protection_level FROM v$database;

prompt <hr><h2>Data Guard Transport/Apply Lag</h2>
SELECT inst_id, name, value
FROM gv$dataguard_stats
WHERE name IN ('transport lag', 'apply lag');

prompt <hr><h2>Data Guard Process Status</h2>
SELECT inst_id, process, status, client_process
